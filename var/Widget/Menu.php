...
class Widget_Menu extends Typecho_Widget
{
    ...
    {
        $parentNodes = array(NULL, _t('后台首页'), _t('主控制台'), _t('内容撰写'), _t('内容管理'), _t('更多设置'),);
		...
		array(
			array(_t('网站概要'), _t('网站概要'), 'index.php', 'subscriber')
        ),
		array(
            ...
            array(_t('数据备份'), _t('数据备份'), 'backup.php', 'administrator'),
        ),
		array(
		    array(_t('撰写文章'), _t('撰写文章'), 'write-post.php', 'contributor'),
            ...
            array(_t('创建页面'), _t('创建页面'), 'write-page.php', 'editor'),
        ...
        array(
            array(_t('管理文章'), _t('管理文章'), 'manage-posts.php', 'contributor', false, 'write-post.php'),
            ...
            array(_t('管理评论'), _t('管理评论'), 'manage-comments.php', 'contributor'),
			...
			array(_t('管理分类'), _t('管理分类'), 'manage-categories.php', 'editor', false, 'category.php'),
			array(_t('管理标签'), _t('管理标签'), 'manage-tags.php', 'editor'),
			...
			array(_t('管理文件'), _t('管理文件'), 'manage-medias.php', 'editor'),
			...
			array(_t('管理页面'), _t('管理页面'), 'manage-pages.php', 'editor', false, 'write-page.php'),
			array(_t('管理用户'), _t('管理用户'), 'manage-users.php', 'administrator', false, 'user.php')
        ),
        
        array(
			array(_t('基本设置'), _t('基本设置'), 'options-general.php', 'administrator'),
			array(array('Widget_Themes_Files', 'getMenuTitle'), array('Widget_Themes_Files', 'getMenuTitle'), 'theme-editor.php', 'administrator', true),
			array(_t('主题设置'), _t('主题设置'), 'options-theme.php', 'administrator', true),
    ...
    {
		$Icon = array(NULL,'home','settings','border_color','inbox','settings_applications');
		$IconColour = array(NULL,'deep-orange','blue','red','orange','pink');
        ...
        foreach ($this->_menu as $key => $node) {
			...
			echo "<div class=\"mdui-collapse-item" . ($key == $this->_currentParent ? ' ' . $class : NULL) . "\">
			        <div class=\"mdui-collapse-item-header mdui-list-item mdui-ripple\">
			        <i class=\"mdui-list-item-icon mdui-icon material-icons mdui-text-color-".$IconColour[$key]."\">".$Icon[$key]."</i>
			        <div class=\"mdui-list-item-content\">{$node[0]}</div>
            ...
            foreach ($node[3] as $inKey => $inNode) {
				...
				echo "<a href=\"" . ($key == $this->_currentParent && $inKey == $this->_currentChild ? $this->_currentUrl : $inNode[2]) . "\" class=\"mdui-list-item mdui-ripple\">{$inNode[0]}</a>";
            }
            echo "</div></div>";
        }
    }
